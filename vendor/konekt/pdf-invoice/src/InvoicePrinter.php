...
class InvoicePrinter extends FPDF
{
    ...
    public function __construct($size = self::INVOICE_SIZE_A4, $currency = '$', $language = 'br')
    {
        $this->items = [];
        $this->totals = [];
        $this->addText = [];
    ...
    public function addItem($item, $description, $quantity, $vat, $price, $discount, $total,$exemption=false)
    {
        $itemColumns = 1;
    ...
    public function Header()
    {
        ...
        if (!isset($this->productsEnded)) {
            ...
            if (isset($this->exemptionField)) {
              
                $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
                $this->Cell(
                    $width_other,
                    10,
                    iconv(self::ICONV_CHARSET_INPUT, self::ICONV_CHARSET_OUTPUT_A, mb_strtoupper($this->lang['exemption'], self::ICONV_CHARSET_INPUT)),
                    0,
                    0,
                    'C',
                    0
    ...
    public function Body()
    {
        ...
        if ($this->items) {
            foreach ($this->items as $item) {
                ...
                if (isset($this->exemptionField)) {
                    $this->Cell($this->columnSpacing, $cHeight, '', 0, 0, 'L', 0);
                    if (isset($item['exemption'])) {
                        $this->Cell($width_other, $cHeight, iconv(self::ICONV_CHARSET_INPUT, self::ICONV_CHARSET_OUTPUT_B, $item['exemption']), 0, 0, 'C', 1);
                    } else {
                        $this->Cell($width_other, $cHeight, '', 0, 0, 'C', 1);
                    }
                }
...

    {
        ...
        //$this->SetTextColor(50, 50, 50);
        if($this->isFinished){
        $this->SetY(-$this->margins['t']+11);
       $footernote = iconv('UTF-8', 'ISO-8859-1', $this->footernote);

        $this->MultiCell(0, -3, $footernote, 0, 'C',0);
        
       // $this->MultiCell(0, 4.5, $this->footerSaftCod, 0, 'C',0);
